...
37. [x] **R14 · 快进三月 + 目标合约 + 单月弹窗帽**  
    「快进三月」会自动花完 AP 后逐月推进；遇强弹窗或 AP 无法花完时立即停下，不越过待处理事项。  
    闯城目标将对应人生合约置顶并标记推荐（落户 ↔ 落户、攒首付 ↔ 首付、向上爬一层 ↔ 升职；还清负债不强推）。  
    每月最多出现 1 个强弹窗，优先级为合约结算 > 要约 > 危机/O1；同一危机不会在短窗内重复。  
    `./scripts/run-fucheng-life-tests.sh` 全绿。

38. [x] **R15 · 快进护栏 + 结算补弹**  
    「快进三月」不会自动执行探区，既有探区目标保持不变；现金偏低时自动行动优先工作，避免继续消费并扩大负债。  
    快进被打断时提示已推进的月数；若已走完最后一个月，不再误报仍有月份未推进。  
    合约结算奖惩只在确认结算选项后入账；刷新或重进时，已判定成功/失败但尚未领取的结算会补弹，且不会重复入账。  
    本月危机满足冷却条件后仍经过概率闸；近期危机去重与每月一个强弹窗上限继续生效。  
    `./scripts/run-fucheng-life-tests.sh` 全绿。

39. [x] **R16 · 危机/O1 刷新补弹 + 快进内置确认**
...
45. [x] **R22 · 焦点 trap 可见过滤 + 选轨/闯城 ARIA**  
